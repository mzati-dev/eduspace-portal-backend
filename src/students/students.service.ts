...
export class StudentsService {
  ...
  /**
   * Recalculates class/qa1/qa2 ranks and writes them to report cards.
   * RANKING FIX: classRank is now computed the SAME way as the class results table:
   * - Uses final-score average (grade config: weighted / end_of_term_only / average_all), not raw end-of-term average.
   * - So "search by exam number" shows the same position as the class results table.
   */
  async calculateAndUpdateRanks(classId: string, term: string, schoolId?: string) {
    ...
    // RANKING FIX: Added 'subject' relation so we can build per-subject data and call calculateFinalScore
    ...
    // 4️⃣ RANKING FIX: Build results using SAME logic as getClassResults (final-score average, not raw endOfTerm)
    // - Build per-subject map with qa1/qa2/endOfTerm + absent flags, then calculateFinalScore per subject
    // - average = sum(finalScore) / subjectCount → same as class results table
    const results: any[] = [];
    ...
    for (const [studentId, items] of byStudent.entries()) {
      ...
      // Same subject structure as getClassResults (for calculateFinalScore + absent handling)
      ...
      for (const subject of subjects) {
        totalScore += this.calculateFinalScore(subject, activeGradeConfig, items);
      }
      const average = subjects.length > 0 ? totalScore / subjects.length : 0;

      // RANKING FIX: Store 'average' (final-score average) instead of 'endRaw' so classRank matches table
      results.push({
        studentId,
        qa1Raw: avg('qa1'),
        qa2Raw: avg('qa2'),
    ...
    // 5️⃣ Dense ranking. RANKING FIX: classRank uses 'average' + same 0.01 tie tolerance as getClassResults
    ...
    // RANKING FIX: classRank = rank by 'average' (final-score average), not by raw endOfTerm — matches class results table
    ...
    for (const sid of studentIds) {
      ...
      // RANKING FIX: classRank now matches position shown in class results and in search-by-exam-number
...
//     // 2️⃣ Load ALL assessments for this class in ONE query (include subject for final-score calculation)
//     const assessments = await this.assessmentRepository.find({
//       where: {
//         class: { id: classId },
//         student: { id: In(studentIds) },
...
//     // 4️⃣ Build results using SAME logic as getClassResults: final-score average (grade config)
//     const results: any[] = [];
...
//     // 5️⃣ Dense ranking: use same tie tolerance (0.01) as getClassResults for classRank
//     const denseRank = (items: any[], field: string) => {
//       const arr = [...items].sort((a, b) => (b[field] ?? 0) - (a[field] ?? 0));
//       const map = new Map<string, number>();
//       let rank = 1;
